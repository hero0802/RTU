...
xh.loadData  = function() {
	...
	app.controller("alarm", function($scope, $http) {
		...
		$http.get("../site/now_data_spd.action?site=" + site + "&province=" + province
...
xh.ExcelToSpd=function(){
	...
	$.ajax({
		...
		success : function(response) {
			xh.maskHide();
			if(response.success){
				var url="../site/DownExcel?excelName=SPD实时数据.xls";	
