...
xh.loadData  = function() {
	...
	app.controller("alarm", function($scope, $http) {
		...
		$scope.count = "20";
		$http.get(
				"../site/h_data_r.action?site=" + site + "&province=" + province
				+ "&city=" + city+"&county="+county + "&company=" + company
				+ "&deviceId="+deviceId+"&model="+model+"&startTime=" + startTime + "&endTime=" + endTime
				+ "&start=" + start + "&limit="
				+ $("#page-limit").val()).
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get("../site/h_data_r.action?site=" + site + "&province=" + province
					+ "&city=" + city+"&county="+county + "&company=" + company
					+ "&deviceId="+deviceId+"&model="+model+"&startTime=" + startTime + "&endTime=" + endTime
					+ "&start=" + start + "&limit="
					+ $("#page-limit").val()).
...
xh.ExcelToRh=function(){
	...
	$.ajax({
		...
		success : function(response) {
			xh.maskHide();
			if(response.success){
				var url="../site/DownExcel?excelName=接地电阻历史数据.xls";	
