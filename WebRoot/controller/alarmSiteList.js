...
xh.loadData  = function() {
	...
	app.controller("alarm", function($scope, $http) {
		...
		$http.get("../site/alarmSiteList?site=" + site + "&province=" + province
		...
		$scope.GoToAlarm=function(id){
			$scope.goData=$scope.data[id];
			window.location.href="alarm.html?siteId="+$scope.goData.siteId
...
/*xh.ExcelToSite=function(){
	...
	var county = $("#selCounty").val();
	var company = $("#company").val();
	window.location.href="../site/ExcelToSite?site=" +
			""+site+"&province="+province+"&city="+city+"&county="+county+"&company="+company+""
}*/
xh.ExcelToSite=function(){
	xh.maskShow();
	var site = $("#site").val();
	...
	$.ajax({
		...
		success : function(response) {
			xh.maskHide();
			if(response.success){
				var url="../site/DownExcel?excelName=站点告警.xls";	
